...
// load the text file containing output from the mapreduce job
// make sure that the file name and path is correct
var fs = require('fs');
...
var uploadFriendRec = function(table, callback) {
  async.forEach(LineArray, function (line, callback) {
	  ...
	  if (kv.length == 2) {
		  ...
		  // split up the different friend names
		  var vs = kv[1].split('~');
		  if (vs.length > 0) {
			  if (vs[0].length > 0) {
				  
		      // store the friend names in a dictionary indexed by
		      // integers (the smaller the integer, the higher the priority)
			  var outV = {};
			  ...
			  var outVString = JSON.stringify(outV);
			   ...
			   // insert an entry into the AWS table
...
function setupFriendRec(err, data) {
  ...
  if (err && h != 2) {
    ...
    setTimeout(setupFriendRec,10000) 
  } else if (h==4) {
