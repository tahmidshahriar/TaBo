...
(function() {
   ...
   * Keys will always be treated as strings.
   * 
   * Callback with a list of objects with keys "key", "value"
   */
  keyvaluestore.prototype.scanKeyVal = function(callback) {
    var self = this;
    
    var params = {
        TableName: self.tableName,
        AttributesToGet: ['keyword', 'value', 'inx']
    };
    ...
    db.scan(params, function(err, data) {
      ...
      if (!err) {
        for (var i = 0; i < data.Count; i++) {
          values.push({
            "key": data.Items[i].keyword['S'],
            "value": JSON.parse(data.Items[i].value['S']),
            "inx" :data.Items[i].inx['N']
          });
        }
      }
